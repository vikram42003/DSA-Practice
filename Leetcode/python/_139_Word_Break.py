...
class Solution:
    # DP (Bottom Up tabs) - Time = O(n ^ 2) - Space = O(n)
    ...
    # we dont go out of bounds), then check the if subsrt[from i to len(word)] matches, if yes then update the i to dp[len(word) + 1
    # idx] and immediately break if a True is found, to ensure we dont overwrite a correct path
