class Solution:
    # Check out the longer comment in your lc problems repo
    # Python stores negative and positive numbers with arbitrary length
    # So we & with mask to make all the bits outside of mask 0 and let us keep our answer within bounds
    # to mantain precision
    # And in the end, a would be the correct 2's complement of the negative number but only upto 32 bits
    # in the final answer we still need to make the bits outside of that 32 length 0, so xor the bits
    # with mask to flip it within the 32 length, ans then NOT it for the final flip
    def getSum(self, a: int, b: int) -> int:
        ...
        if a > mask >> 1:
            ...
            # (...1111111)11111111111111111111111111111101
            return ~(a ^ mask)
        else:
            return a
